...
|  | Temperature gradients | °C, K | ΔT<sub>core–skin</sub>, ΔT<sub>neck–ankle</sub>, ΔT<sub>prox–dist</sub>, DPG | {ref}`Distal–proximal gradient <label-distal-proximal-gradient>` widely used in {ref}`thermoregulation <label-thermoregulation>` studies |
...
|  | {ref}`Tonic <label-tonic>` component | µS, mS | SCL | Skin conductance level ({ref}`baseline <label-baseline>` component) |
|  | {ref}`Phasic <label-phasic>` component | µS | SCR | Skin conductance response (transient responses) |
...
|  | Mean {ref}`arterial pressure <label-arterial-pressure>` | mmHg | MAP, P<sub>m</sub> | Often approximated from SBP and DBP |
|  | Stroke volume | mL | SV | — |
|  | {ref}`Cardiac output <label-cardiac-output>` | L·min<sup>−1</sup> | CO, Q̇ | CO most common in clinical literature |
...
|  | {ref}`Heat storage <label-heat-storage>` | kJ·kg<sup>−1</sup>, W | S | Represents change in body heat content |
